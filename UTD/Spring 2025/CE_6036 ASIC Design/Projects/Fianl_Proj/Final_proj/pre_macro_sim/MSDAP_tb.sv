...
module MSDAP_TB;
  ...
  parameter T_Sclk = 1302;
  parameter T_Dclk = 33;
  parameter M_input = 6394;
  
  MSDAP uut( .Sclk(Sclk), .Dclk(Dclk), .Start(Start), .Reset_n(Reset_n), .Frame(Frame), .InputL(InputL), .InputR(InputR), .InReady(InReady), .OutReady(OutReady), .OutputL(OutputL), .OutputR(OutputR) );
...
#2  Start=0;
    ...
    fd = $fopen("/home/eng/i/dal852207/cad_ASIC/genius_syn/Final_proj/pre_macro_sim/data.in", "r");
    if(fd) 
        $display("Successfully read in data.in.");
    else begin
        $display("Failed to read in data.in.");
        $finish;
    end
    
    repeat(4) begin
        $fgets(line,fd); // Skip 4 lines at the beginning
        //$display("@[data.in] %s",line);
    end 
  ...
  initial begin
    fd2 = $fopen("/home/eng/i/dal852207/cad_ASIC/genius_syn/Final_proj/pre_macro_sim/data2.out", "r");
    if(fd2) 
