...
module MSDAP_tb;
	...
	reg [4:0] shift_counter = 0;   // Counts 0~15 -> shift in 16 bits
	...
	always @(posedge Dclk) begin
		if (((dataindex == 9458) || (dataindex == 13058)) && !reg_resetflag) begin
			Reset_n <= 1'b0;
			reg_resetflag <= 1'b1;
			sending <= 0;
			Frame <= 0;
		end
		else if (InReady || reg_resetflag) begin
			Reset_n <= 1'b1;

			if (!sending) begin
				// Load new data, assert Frame
				if (dataindex < 15056) begin
					InputL <= data[dataindex];
					InputR <= data[dataindex+1];
					dataindex <= dataindex + 2;
					Frame <= 1'b1;  // Assert Frame exactly when loading
					sending <= 1'b1;
					shift_counter <= 15;
				end
			end
			else begin
				// Continue shifting ? Frame low during shift
				Frame <= 1'b0;

				if (shift_counter > 0)
					shift_counter <= shift_counter - 1;
				else begin
					sending <= 0;  // Done shifting ? ready for next
					if (reg_resetflag)
						reg_resetflag <= 1'b0;
				end
			end
