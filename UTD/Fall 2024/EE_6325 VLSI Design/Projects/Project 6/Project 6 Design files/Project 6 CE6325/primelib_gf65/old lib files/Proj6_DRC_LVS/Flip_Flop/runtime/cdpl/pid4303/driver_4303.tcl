if { [catch {
set_location /home/eng/d/dal852207/cad/gf65/primelib_gf65/Proj6_DRC_LVS/Flip_Flop
log_info "[clock format [clock seconds]]: Sourcing environment settings"
set t0 [clock seconds]
set sis_log_file "/var/tmp/pl.871870.W$::env(CDPL_WORKERID).[lindex [split [info hostname] '.'] 0].[pid].log"
if { [info exists ::env(SGE_TASK_ID)] } {
  if { $::env(SGE_TASK_ID) != "undefined" } {
    set sis_log_file "/var/tmp/pl.871870.W$::env(SGE_TASK_ID).[lindex [split [info hostname] '.'] 0].[pid].log"
  }
}
...
set_config_opt -- cdpl_log_subdir pid4303

create_parameter configure_arc_based
set_config_opt -cell Flip_Flop -- configure_arc_based 0
...
if {[catch {
  ...
  reset_log_file
  if { [log_num_errors] > 0 } {
    if { [catch {exec mv -f $sis_log_file /home/eng/d/dal852207/cad/gf65/primelib_gf65/Proj6_DRC_LVS/Flip_Flop/runtime/cdpl/pid4303} mv_err] } {
      log_warning $mv_err
    }
  } else {
