...
void tb::sink() {
	...
	sprintf( outfile, "./output.dat" );
	// if output.dat does not open notify the user
	if (outfp == NULL)
	{
	...
	for( int i = 0; i < 64; i++ )
	{
		...
		//wait();
		outp_rdy.write ( 0 );
		// make sure to use file print function (fprintf) to write the indata to the outfp file in int form 
		fprintf( outfp, "%d\n", (int)indata );
		cout << i << " :/t" << indata.to_int() << endl;		//print current i of indata to an integer and end line
	}
}
//vlose the file before you end simulation
fclose( outfp );
